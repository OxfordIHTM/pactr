...
[![Codecov test coverage](https://codecov.io/gh/OxfordIHTM/pactr/graph/badge.svg)](https://app.codecov.io/gh/OxfordIHTM/pactr)
...
pact_data_raw <- pact_read_website()
pact_data_raw
```
...
disease_levels <- tidy_df$Diseases |>
  unique() |>
  sort() |>
  (\(x) x[!x %in% c("Other", "Unspecified", "Not applicable", "Congenital infection caused by Zika virus")] )() |>
  c("Congenital infection caused by Zika virus", "Other", "Unspecified", "Not applicable")

disease_labels <- c(
  "Argentine HF", "Klebsiella pneumonia", "Chikungunya", "Cholera",
  "COVID-19", "CCHF", "Dengue", "Disease X", "Venezuelan equine encephalitis", 
  "Ebola", "Hendra virus", "Nipah virus", "Influenza A H2", "Influenza A H5",
  "Influenza A H7", "Lassa fever", "Lujo fever", "Marburg", "MERS-CoV", "Mpox",
  "Plague", "RVF", "Salmonella", "SARS", "Severe fever & thrombocytopenia",
  "Shigellosis", "Smallpox", "Tick-borne encephalitis", "Vaccinia", "West Nile",
  "Yellow Fever", "Zika", "Zika (congenital)",
...
disease_df <- tidy_df |>
  dplyr::mutate(
    Diseases = factor(
      x = Diseases, levels = disease_levels, labels = disease_labels
    )
  )
...
ggplot2::ggplot(
  data = disease_df,
  mapping = ggplot2::aes(
    x = GrantStartYear, y = n_grants, group = Diseases, colour = Diseases
  )
) +
  ggplot2::geom_line() +
...
```{r usage-website-3d, echo = FALSE, fig.height = 20}
disease_df |>
  ggplot2::ggplot(
    mapping = ggplot2::aes(x = GrantStartYear, y = n_grants, group = Diseases)
  ) +
  ggplot2::geom_line(
  ...
  ggplot2::facet_wrap(. ~ Diseases, nrow = 9, ncol = 4, scales = "free_y") +
  ggplot2::labs(x = "Year of Award Start", y = "Number of Grants") +
  ggplot2::theme_bw() +
  ggplot2::theme(
    ...
    strip.text = ggplot2::element_text(colour = "#FFFFFF", size = 8),
    panel.border = ggplot2::element_blank(),
    panel.grid.minor = ggplot2::element_blank(),
