...
pact_cite <- function(pact_client = pact_client_set(),
                      id) {
  ...
  paste0(
    'bibentry(
      ...
      title    = "', info$title, '. ', info$defined_type_name, ' version ', info$version, '",
      author   = ', process_author_names(info$authors$full_name), ',
      year     = "', strptime(info$published_date, format = "%Y") |> format(format = "%Y"), '",
      doi      = "', info$doi, '"
    )'
  ),
  path)
